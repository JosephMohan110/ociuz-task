...
CREATE TABLE IF NOT EXISTS tblDocumentStatusMaster (
    ...
    -- SequenceOrder (1, 2, 3): The ladder order. It ensures a user cannot skip from DRAFT straight to APPROVED without going through PENDING first.
    ...
    -- ColorCode (#22c55e): The UI dress code. The frontend reads this code directly from the database to paint the badges automatically.
...
RETURNS INT AS $$
DECLARE
    v_new_leave_id INT;   --Stores newly created leave request ID.
    v_doc_number VARCHAR;   --Stores generated ERP number.
    v_initial_status VARCHAR;
BEGIN
...
--VALIDATION
    ...
    --Calls another function to ernate number..
